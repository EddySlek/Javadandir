...
public class FunctionalProgramming01 {
	...
	/*
	 	...
	 	5)Lambda does not create method. Just use existing method in Java. Like String, Array, Math or UTILS classes, etc
	 	  And Lambda is fast, less coding.
	 */
    ...
    //2)Create a method to print the even list elements on the console in the same line 
	//with a space between two consecutive elements.(Structured)
	...
	public static void sumOfSquareDistinctEvens(List<Integer> l) {
		...
		//For sum, make the first parameter zero t:firstparameter, u:seconparameter. LOOK Lambda1 Picture
	}
	...
	public static void getHalfOfDistinctElReversed(List<Integer> l) {
		...
		System.out.println("Half of the elements list: " + resutList);//[65.5, 7.5, 7.0, 6.0, 5.0, 4.5]
		//collect(Collectors.toList())=> return a List. 
		
		List<Integer> resutList2 = l.stream().distinct().filter(t->t>5).map(t->t/2).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
		System.out.println("Half of the elements list: " + resutList2);//[65, 7, 7, 6, 5, 4]
	}
}